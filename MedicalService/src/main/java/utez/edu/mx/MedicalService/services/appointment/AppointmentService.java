...
public class AppointmentService {
    ...
    @Transactional(rollbackFor = {SQLException.class})
    public CustomResponse<Appointment> updateStatus(AppintmentUpdateStatusDTO updateStatusDTO, BindingResult bindingResult){
        try {
            if(!this.repository.existsById(updateStatusDTO.getId_appointment())){
                return new CustomResponse<>(null,true,400,"La cita no existe");
            }else if (bindingResult.hasErrors()) {
                // Si hay errores de validación, devuelve una respuesta con los mensajes de error
                String errorMessage = convertErrors.convertErrorsValidationToString(bindingResult);
            ...
            this.repository.updateStatusById(updateStatusDTO.getId_appointment(),updateStatusDTO.getId_new_status());
            return new CustomResponse<>(null,false,200,"El estatus de la cita se actualizao correctamente");
        }catch (Exception e){
            return new CustomResponse<>(null,true,400, "Error al eliminar la cita");
        }
    }
